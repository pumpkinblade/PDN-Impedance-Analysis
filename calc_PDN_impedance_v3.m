...
%(to be improved)
T_grid      = calc_T_grid(himd,cond_si,esi,epoly,cond_cu,tand_poly,t_metal,grid_width,grid_spacing,M_grid,N_grid,in_port_grid,out_port_grid,freq,25);
...
figure
h = gca;
loglog(freq,abs(Z_PDN),'LineWidth',2)
hold on
loglog(freq,abs(Z_PCB_SA),'LineWidth',1)
loglog(freq,abs(Z_C4_SA),'LineWidth',1)
loglog(freq,abs(Z_TSV_SA),'LineWidth',1)
loglog(freq,abs(Z_via_SA),'LineWidth',1)
loglog(freq,abs(Z_ubump_SA),'LineWidth',1);
loglog(freq,abs(Z_grid_SA),'LineWidth',1);
